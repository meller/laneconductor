...
describe('Track TU-10049 Phase 6: real Launch with a Jira connection configured', () => {
  ...
  after(async () => {
    ...
    // from under it, and the orphan keeps running indefinitely (confirmed
    // live: ps showed real `--sync-only` processes with this exact fixture
    // path "(deleted)" as their cwd, still burning 100%+ CPU). stopWorker
