...
const server = createServer(async (req, res) => {
  ...
  // .catch(() => null) this, so leaving it unhandled never THROWS — but it
  // silently changes their behavior (e.g. seedCursorFromDB always sees
  // remoteCount 0), which was enough to stall lane-transition tests that
  // otherwise have nothing to do with either of those two call sites.
  if ((params = route('GET', '/track/:num', req)) !== null) {
    const t = state.tracks[params.num];
    if (!t) return reply(res, 404, { error: 'track not found' });
    const comments = state.comments.filter(c => String(c.track_number) === params.num);
  ...
  // /project/:id/dispatch/claimed-by-offline-workers (ui/server/index.mjs)
  // — lets one worker's reconciler see dispatches claimed by a DIFFERENT,
  // now-offline worker identity, not just its own claims. state.offlineWorkerIds
  // (a plain array, set via POST /_set-offline-workers below) stands in for
  // the real server's `last_heartbeat < NOW() - INTERVAL '60 seconds'` check.
  if ((params = route('GET', '/project/:id/dispatch/claimed-by-offline-workers', req)) !== null) {
    const offlineIds = state.offlineWorkerIds || [];
    const entries = state.dispatch.filter(d => offlineIds.includes(d.worker_id) && d.status === 'claimed');
    return reply(res, 200, { entries });
  }
  ...
  // Mirrors the real server's POST /track/:num/prespawn-block and its
  // /reset counterpart (ui/server/index.mjs) — the worker calls /reset
  // unconditionally after most lane actions, so leaving it UNHANDLED here
  // stalls the whole lane transition the test is waiting on, not just this
  // one call.
  if ((params = route('POST', '/track/:num/prespawn-block', req)) !== null) {
    const { num } = params;
    if (!state.tracks[num]) state.tracks[num] = { track_number: num, fail_count: 0 };
    const t = state.tracks[num];
    t.prespawn_block_count = (t.prespawn_block_count || 0) + 1;
    t.prespawn_block_kind = body.kind ?? null;
    t.prespawn_block_reason = body.reason ?? null;
    return reply(res, 200, { count: t.prespawn_block_count, kind: t.prespawn_block_kind, reason: t.prespawn_block_reason });
  }

  if ((params = route('POST', '/track/:num/prespawn-block/reset', req)) !== null) {
    const { num } = params;
    if (state.tracks[num]) {
      state.tracks[num].prespawn_block_count = 0;
      state.tracks[num].prespawn_block_kind = null;
      state.tracks[num].prespawn_block_reason = null;
    }
    return reply(res, 200, { ok: true });
  }
