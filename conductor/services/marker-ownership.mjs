...
export const AUTHOR_OWNED_MARKERS = Object.freeze([
  ...
  'Workspace',
  'Depends On',
  // Track AM-10099 Phase 11 Task 3 (item l): a per-track model override —
  // set only via the track detail panel's dedicated "Model override"
  // field (workflow.md's Model Overrides section, tier 1) — is exactly as
  // deliberate a human action as Merge Mode/Workspace/Auto Run. Its write
  // site (syncTrackToFile's `updates.model_override` branch) had no
  // provenance guard at all before this fix, unlike its three siblings.
  'Model',
]);
...
export const MACHINE_OWNED_MARKERS = Object.freeze([
  ...
  'Progress',
  'Phase',
  // Track AM-10099 Phase 11 Task 3 (item l): both DB<->FS writers already
  // touch these; they were simply never added to either table, which is
  // its own hole in the classification a writable-but-unclassified marker
  // could silently fall through. `Summary` is regenerated by every lane
  // action (SKILL.md's marker table: "A one-line summary of the current
  // work/problem") — track 1081's incident with it was a truncation bug
  // in the sync pipeline (fixed separately in summary-utils.mjs), not an
  // authorship violation; it belongs here, next to `Phase`, not in the
  // author-owned table. `Waiting Reason` travels with `Lane Status:
  // waiting` (track 10055) and is cleared/rewritten by whichever action
  // parks or resumes the track — same lifecycle as Lane Status itself.
  'Summary',
  'Waiting Reason',
]);
