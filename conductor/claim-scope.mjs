 ...
 * bypasses `claimableSet` for tracks mid-conversation, so a track already
 * being answered doesn't get stranded by assignee gating. The allowlist is
 * NOT bypassed the same way — if it were, a worker scoped to track 42 would
 * still answer arbitrary other tracks, and the scoping guarantee would be
 * worthless exactly when it matters.
 */
export function isTrackClaimable(trackNumber, { claimableSet = null, onlyTracks = null, waitingForReply = false } = {}) {
