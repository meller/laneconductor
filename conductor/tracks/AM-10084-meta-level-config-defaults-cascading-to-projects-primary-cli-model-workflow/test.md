...
- [ ] TC-1.1: `loadMetaDefaults()` with no `meta-defaults.json` on disk at
      `META_PROJECT_REPO_PATH` — returns `{}`, does not throw.
- [ ] TC-1.2: `loadMetaDefaults()` with a malformed JSON file — returns `{}`,
      does not throw, matches `conductor/defaults.json`'s existing
      warn-and-continue behavior.
- [ ] TC-1.3: `loadMetaDefaults()` with a valid file — returns the parsed
      object unchanged.
- [ ] TC-1.4: `mergeEffectivePrimary` — CLI flag override wins over every
      other tier even when project config, project defaults, and meta
      defaults all disagree.
- [ ] TC-1.5: `mergeEffectivePrimary` — project's own `.laneconductor.json`
      wins over project's own `conductor/defaults.json`, meta defaults, and
      hardcoded, when no CLI flag is given.
- [ ] TC-1.6: `mergeEffectivePrimary` — project's own `conductor/defaults.json`
      wins over meta defaults and hardcoded, when project config is silent.
- [ ] TC-1.7: `mergeEffectivePrimary` — meta defaults win over hardcoded when
      every project-level tier is silent.
- [ ] TC-1.8: `mergeEffectivePrimary` — hardcoded `'claude'` is the result
      when every tier including meta defaults is silent.
- [ ] TC-1.9: `mergeEffectivePrimary` with `inheritMetaDefaults: false` —
      ...
      silent).
- [ ] TC-1.10: `mergeWorkflowConfig` — project's `conductor/workflow.json`
      overriding only `lanes.review.max_retries` still yields every other
      lane's settings from the meta `workflow` block.
- [ ] TC-1.11: `mergeWorkflowConfig` — for a lane meta doesn't mention at all,
      falls through to `globalCanonicalWorkflow`'s value for that lane.
- [ ] TC-1.12: `mergeWorkflowConfig` — project's own value for a given
      lane+key always wins over the same lane+key in meta defaults, even
      when both are set.
- [ ] TC-1.13: `mergeWorkflowConfig` with `inheritMetaDefaults: false` —
...
- [ ] TC-2.1: With `conductor/meta-defaults.json` present at
...
- [ ] TC-2.2: Same setup, but the test project's own `.laneconductor.json`
      sets `primary.cli: "antigravity"` — confirm the resolved value is
      `'antigravity'`, not the meta default.
- [ ] TC-2.3: Same setup, but the test project sets
...
- [ ] TC-2.4: Touching `conductor/workflow.json` while the worker is running
      (existing chokidar reload path) re-runs the merge — confirm a lane
      added to the project file after the process starts inherits from meta
      defaults on the very next reload, not only at process start.
- [ ] TC-2.5: `lc workflow` (bin/lc.mjs's display command) prints the same
...
- [ ] TC-2.6: `lc state --json`'s reported primary CLI for a project with no
...
- [ ] TC-4.1 (parity): every existing test in
      ...
      unchanged.
- [ ] TC-4.2: `workerMode: 'manager-driven'`, `hasOnlineWorker: false`,
...
- [ ] TC-4.5: `workerMode: 'dedicated'` (explicit, not defaulted),
...
- [ ] TC-5.1: `GET /api/state?project_id=<livingwork-like-fixture>` against
...
- [ ] TC-5.2: Same fixture but with zero `provider_status` rows for `claude`
      anywhere in the DB — response's `gaps` DOES contain blocking
      `no-provider`.
- [ ] TC-5.3 (manual, real instance): livingwork's actual
...
- [ ] All automated test cases above pass.
- [ ] TC-5.3's manual real-instance check performed and its observed result
      recorded in `conversation.md` (screenshot or the actual `/api/state`
      JSON response), per this project's own quality-gate rule that a
      user-facing claim needs a real observation, not just passing unit
      tests.
- [ ] No regressions in `conductor/tests/track-10069-setup-gaps.test.mjs`'s
      original assertions.
- [ ] `conductor/workflow.md` updated and accurate against the shipped
