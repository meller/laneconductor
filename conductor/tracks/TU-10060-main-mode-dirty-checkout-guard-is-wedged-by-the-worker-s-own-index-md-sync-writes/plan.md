...
- [x] Task 1: Add `conductor/tests/track-10060-dirty-guard-exemptions.test.mjs`
...
- [x] Task 2: Assert the same for a `--untracked-files=all` shaped list, so
      the per-file expansion (`??` lines for a brand-new track folder) is
      covered alongside modified files.
- [x] Task 3: Add a short comment block at the guard site
...
- [x] Task 1: Extract the filesystem counter read/write from the `local-fs`
      ...
      semantics (REQ-3).
- [x] Task 2: In the collector branch's `catch`, call that helper instead of
...
- [x] Task 3: Emit a distinct one-per-streak warning on counter-backend
...
- [x] Task 4: Extend the two reset sites
...
- [x] Task 1: Change `formatBlockComment` in
...
- [x] Task 2: Keep `formatBlockComment` a pure function with no new inputs
      beyond the existing outcome object, so it stays unit-testable and the
      other block kinds' wording is untouched.
- [x] Task 3: Update `conductor/tests/track-10040-prespawn-block.test.mjs`
...
- [x] Task 3: Thread `suggestion` into the guard's existing `healSuggestion`
      string so it reaches the block comment, alongside (not replacing) the
      existing `remedy` suggestions.
- [x] Task 4: Verify by inspection that no existing caller treats a truthy
...
- [x] Task 1: Run the full node:test suite for the touched modules
...
- [x] Task 2: Run the escalation end to end against a real dirty checkout
...
- [x] Task 3: Restart the worker before observing behaviour — it does not
