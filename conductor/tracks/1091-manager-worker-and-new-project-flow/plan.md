...
- [ ] Task 1: Migration — `ALTER TABLE workers ADD COLUMN type TEXT DEFAULT 'project'`
- [ ] Task 2: API validation — `create-project` dispatch creation rejects a `worker_id` whose `type != 'manager'`

## Phase 2: CLI

**Problem**: No way to start a worker as a manager.
**Solution**: `--manager` flag on `lc worker start`.

- [ ] Task 1: `lc worker start --manager` — `POST /worker/register` sends `type: 'manager'`
- [ ] Task 2: Confirm combinability with existing `--sync-only`/`--worker-number` flags (1084)
- [ ] Task 3: Omitting the flag registers `type: 'project'` (default, backward compatible)
