...
- [ ] Confirm `prisma/schema.prisma` already declares `waiting_for_reply`, `auto_run`,
...
- [ ] Confirm each has a corresponding file under `migrations/` (already confirmed during
...
- [ ] Note in the implement-phase conversation.md comment whether the actual deployed cloud DB
...
**Impact**: confirms this is a pure application-code change; no `migrations/*.sql` file is
added by this track.
...
- [ ] Add `waiting_for_reply, auto_run` to the destructured request body in
      `cloud/functions/index.js`'s `POST /track` handler.
- [ ] Add both to the INSERT column list and VALUES, using
...
- [ ] Add both to `ON CONFLICT DO UPDATE SET` as
...
- [ ] Add `merge_mode, workspace_mode` to the destructured request body.
- [ ] Add both to the INSERT column list/VALUES (`merge_mode ?? null`, `workspace_mode ?? null`).
- [ ] Add both to `ON CONFLICT DO UPDATE SET` as
...
- [ ] Carry over the explanatory comments from `ui/server/index.mjs` (why raw-nullable, why
...
- [ ] Add `track_type, kpi_target, kpi_actual, kpi_metric, kpi_source, kpi_source_config,
...
- [ ] Add all 13 to the INSERT column list/VALUES:
      `track_type ?? 'dev'`, `kpi_snapshot ? JSON.stringify(kpi_snapshot) : null`, the rest
      `field ?? null`.
- [ ] Add all 13 to `ON CONFLICT DO UPDATE SET`:
...
- [ ] Add `model_override` to the destructured request body.
- [ ] Add to INSERT column list/VALUES (`model_override ?? null`).
- [ ] Add to `ON CONFLICT DO UPDATE SET`:
      `model_override = COALESCE(EXCLUDED.model_override, tracks.model_override)`.
- [ ] Add a code comment explaining this is deliberately NOT a copy of local's `POST /track`
...
- [ ] Write/extend `cloud/functions/test/track-10085-post-track-field-parity.test.js` (or the
...
- [ ] Per field in Phases 1-4: assert insert-time value, update-time payload-wins, and
...
- [ ] Run `conductor/tests/cloud-route-parity.test.mjs` and confirm it still passes unmodified.
- [ ] Run the full existing `cloud/functions/test/` suite to confirm no regression in
