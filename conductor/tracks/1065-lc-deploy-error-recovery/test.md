...
- [x] TC-1: A step printing 250 numbered lines then exiting 1 — expected:
      `result.outputTail` contains the last 100 lines, including the final line,
      and does **not** contain line 1.
- [x] TC-2: A step writing to **stderr** then exiting 1 — expected: the stderr
      text appears in `outputTail` (stdout and stderr are interleaved, not
      stdout-only).
- [x] TC-3: A step exiting 0 — expected: `result.ok === true` and the return
      shape is unchanged from today (`exitCode`, `logFile`, `url`).
- [x] TC-4: Live streaming with `echo: true` — a step printing a line, sleeping
      ...
      step end).
- [x] TC-5: The existing `deploy-runner.test.mjs` suite passes unchanged —
      expected: same pass count as before this track's changes.

### Phase 2 — `buildDeployRecoveryPrompt`
- [x] TC-6: Prompt includes the failed step label, the exact command string, and
      the output tail — expected: all three substrings present.
- [x] TC-7: `deployStackMd` provided — expected: its content appears in the
      prompt.
- [x] TC-8: `deployStackMd` null/empty (no `conductor/deployment-stack.md`) —
      expected: no throw, and no empty "Deployment Stack" heading left behind.
- [x] TC-9: `deployEnvConfig` provided — expected: the environment's
      `deploy.json` entry appears in the prompt.
- [x] TC-10: `history` with two prior turns — expected: both turns appear, in
      order, so a refinement round is multi-turn.
- [x] TC-11: Rules block — expected: prompt instructs ending with exactly
...
- [x] TC-12: Fake `ask` returns `''` (Enter) on the first prompt — expected:
      returns `'verify'` after exactly one `callLLM` call.
- [x] TC-13: Fake `ask` returns `'q'` — expected: returns `'abort'`.
- [x] TC-14: Fake `ask` returns `'why does auth fail?'` then `''` — expected:
      `callLLM` called twice, the second prompt contains the follow-up text, and
      the loop then returns `'verify'`.
- [x] TC-15: Fake `ask` returns `'r'` then a question then `''` — expected: `r`
      is treated as "discuss more" and prompts for the question rather than being
      sent verbatim as the refinement.
- [x] TC-16: The captured output tail is written through the injected `write` —
      expected: the failure header and the tail both reach `write`.

### Phase 3 — Verification gate
- [x] TC-17: Two-step `commands` array, step 1 passes, step 2 fails once then
      ...
      once** (not re-run).
- [x] TC-18: Three-step array, step 2 fails then passes on verify — expected:
      step 3 runs afterward and `result.ok === true`.
- [x] TC-19: Step always fails, hook always returns `'verify'` — expected: the
      ...
      `ok: false`.
- [x] TC-20: Hook returns `'abort'` on the first call — expected: `ok: false`,
      `aborted: true`, the command ran exactly once, and later steps did not run.
- [x] TC-21: Single `command` string (not an array) that fails then passes on
      verify — expected: `ok: true`, recovers identically to the array shape.
- [x] TC-22: **No `onStepFailure` passed** (the worker-dispatch path) on a
...
- [x] TC-23: Hook throws — expected: `runDeploy` does not leave the log stream
      open or hang; it returns a failure result.

### Phase 4 — CLI integration
- [x] TC-24: `lc deploy <env> --no-recovery` against a failing step — expected:
      non-zero exit, no prompt, error and log path printed. Run as a real
      subprocess with stdin closed.
- [x] TC-25: `lc deploy <env>` with stdin **not** a TTY against a failing step —
      expected: non-zero exit, no prompt, and a line stating recovery was
      skipped because the session is non-interactive.
- [x] TC-26: `lc --help` — expected: the `deploy` entry documents
      `--no-recovery`.

### Manual / real-product check (required before quality-gate)
- [x] TC-27: With a temp `deploy.json` whose step runs a command that fails for a
...
- [x] `conductor/tests/deploy-runner.test.mjs` shows no new failures versus
      `main` (diff-confirmed, not assumed) — 23/23 pass, up from 17/17 on main (6 new tests added, none regressed).
- [x] No recovery prompt can ever be reached on the worker's dispatch path — verified: onStepFailure is undefined at both worker call sites in conductor/laneconductor.sync.mjs, and runDeploy's own test suite confirms the hook is never invoked when absent.
- [x] TC-27 performed, with the observation recorded in `conversation.md`.
