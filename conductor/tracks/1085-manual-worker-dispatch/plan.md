...
- [x] Task 4: Two workers, dispatch targeted at worker A never runs on worker B — `conductor/tests/track-1085-dispatch-worker.test.mjs`, two real worker processes (`--worker-number 1`/`2`, same project) via `startWorkerCapturing`; asserts on each worker's own captured log (worker A's mentions the dispatch, worker B's never does) rather than just trusting the API-level scoping. Caught a real test-timing bug while writing it: the first draft used a fixed `sleep(3000)` before checking the dispatch entry's final status, which raced `reconcileActiveDispatch`'s independent 5s poll interval and flaked (`'claimed' !== 'done'`) — fixed by polling for the status to settle, matching Tasks 1/3's existing pattern, instead of guessing a fixed delay.
