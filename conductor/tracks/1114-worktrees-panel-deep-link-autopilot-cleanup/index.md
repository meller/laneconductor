...
- [x] Phase 15: Discard track (no merge) — found live doing exactly this by hand for a real track (macrodash #031, abandoned after a product-direction change away from PayPal). New `discard-track` dispatch action (`conductor/laneconductor.sync.mjs`): removes the worktree (branch/commits stay recoverable, same guarantee as Remove Worktree), moves the track to `backlog:queue` with an explicit abandonment note written into `index.md`'s Summary (never `done:success`, which would misrepresent it as shipped), syncs the lane to the DB, posts a conversation comment. "Discard (no merge)" armed-confirm button wired in `WorktreesPanel.jsx` on every row with a track. Landed via commits `316c4e3`/`6e26143` (merged into this branch through the main-reconcile merge, `7d16ac1`) — **found during this quality-gate pass to be fully implemented and wired end-to-end (API route allow-list, dispatch handler, UI button) but never marked done here; checkbox corrected, no code change needed.**
- [x] Phase 16: "No worker available" refresh failure recovery path — found live: the empty "No Unmerged Worktrees" state's refresh could fail with "no worker available for this project to refresh worktrees" (Phase 13's error banner correctly surfaced this, but then the user was stuck). Fixed with a "Go to Workers →" deep-link button (`onGoToWorkers`, threaded `WorktreesPanel` → `App.jsx` → `setViewMode('workers')`) shown whenever a refresh fails specifically with that "no worker available" message, in both the stats-header and empty-state error banners. Landed via commit `6e26143` (same merge as Phase 15). **Found during this quality-gate pass to be fully implemented and wired end-to-end; checkbox corrected, no code change needed.**
