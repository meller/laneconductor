...
node --test conductor/tests/claude-cli-args.test.mjs

# Phase 5/6
node --test conductor/tests/track-1111-model-staleness.test.mjs
node --test conductor/tests/track-1111-phase6-auto-update.test.mjs
```

## Test Cases

### Phase 1: Per-lane model, live validation
- [x] TC-1: With `primary_model` set on all 5 lanes of this project's
      `workflow.json`, a real dispatch to each lane shows that lane's
      configured model in the actual `--model` flag / transcript — not
      inferred, observed
- [x] TC-2: Across all 4 automated lanes, `chosenCli`/the provider stays
      identical — no provider switch mid-track (REQ-3)
- [x] TC-2b: The Phase 1 Task 5 guard actually fires — a `workflow.json`
...
- [x] TC-3: A `track_chat` dispatch against a track currently in a lane
      with a configured `primary_model` uses that model (once Phase 2's
      decision is implemented)
- [x] TC-4: A `worker_adhoc_chat` dispatch (no track) uses the project
...
- [x] TC-5: Lane HAS a configured `primary_model` + a manual per-worker
      override is active (via 1096's `set_model`) → the lane's model
      wins for an automated dispatch (REQ-2)
- [x] TC-6: Lane has NO configured `primary_model` + a manual override is
      active → the manual override is used (correct fallback, not a
      silent no-op)

### Phase 4: Cross-project audit
- [x] TC-7: Every actively-used project's `workflow.json` (per Phase 4's
...
- [x] TC-8: A `workflow.json` entry naming a model absent from that
...
### Phase 6: Auto-update
- [x] TC-9: A project that has NOT opted in is never auto-updated, even
      when a staleness condition is detected (default-off verified) —
      `track-1111-phase6-auto-update.test.mjs`, "opt-in gate (TC-9)" suite
      (3 cases: unset, explicit `false`, opted-in-but-nothing-to-apply)
- [x] TC-10: A project that HAS opted in gets a same-tier version bump
      only — cross-tier substitution (opus↔sonnet) never happens, and the
      change is present in `workflow.json`'s git history —
      `track-1111-phase6-auto-update.test.mjs`, "opted-in same-tier
      update (TC-10)" suite plus the real-temp-git-repo integration test
      confirming the change actually lands in `git log`/`git show`

## Acceptance Criteria
- [x] All test cases above pass
- [x] No regressions in `local-fs-e2e.test.mjs`, `local-api-e2e.test.mjs`,
      `track-1087-worker-chat-dispatch.test.mjs`,
      `track-1086-session-worker.test.mjs` (the one `local-api-e2e.test.mjs`
      failure is a pre-existing flake, confirmed by reproducing it
      identically with this track's changes stashed out — see
      conversation.md)
- [x] Every REQ in spec.md has a corresponding verified test case or
