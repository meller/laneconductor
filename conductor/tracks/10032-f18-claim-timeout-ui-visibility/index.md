...
## Phases
- [x] Phase 1: Record the reap outcome durably — `worker_dispatch.reaped_at` + `reap_reason` (migration `011_dispatch_reap.sql`), written on both the reassign and fail branches
- [x] Phase 2: Push a track-scoped reap into the Inbox as a `system` ⚠️/❌ comment + `track:updated` broadcast
- [x] Phase 3: Annotate the per-track dispatch history strip (`TrackDetailPanel`) — reassignment is currently indistinguishable from a healthy pending dispatch
- [x] Phase 4: Annotate the project-level CI/CD dispatch history (`CICDView`) — the only surface for `track_number IS NULL` dispatches
- [x] Phase 5: Real test coverage — mocked-pool unit tests, a supertest API-shape test, and a fast-tier Playwright check that it renders
