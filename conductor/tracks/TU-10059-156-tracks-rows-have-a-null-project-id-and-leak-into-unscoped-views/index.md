...
**Summary**: 156 of 811 tracks rows have project_id IS NULL. Root cause found: POST /track (ui/server/index.mjs:2677) resolves projectId to null when no token or project_id is given, and ON CONFLICT (project_id, track_number) never fires on a NULL, so the upsert degrades to an insert. Not a single import (13 days, Mar-Aug) and they do NOT leak into any view (every unscoped read inner-joins projects). All 156 verified safe to delete: 0 comments, 0 locks, 7 fixture-only orphans. Plan: guard the write path, delete the rows, make project_id NOT NULL, fix the declared-vs-live schema drift.
**Merge Mode**: direct
**Auto Run**: yes
