...
- [ ] TC-1a: `parseVerdict` returns `'pass'` for `**Verdict**: pass`
- [ ] TC-1b: `parseVerdict` returns `'fail'` for `**Verdict**: fail`
- [ ] TC-1c: `parseVerdict` is case-insensitive (`**Verdict**: PASS`, `**Verdict**: Fail`)
- [ ] TC-1d: `parseVerdict` returns `null` when the marker is absent
- [ ] TC-1e: `parseVerdict` returns `null` for an empty marker value (`**Verdict**:   `)
- [ ] TC-1f: `parseVerdict` returns `null` for an unrecognized value (`**Verdict**: maybe`) —
      never guessed
- [ ] TC-1g: `writeVerdict` updates an existing marker in place (no duplicate line)
- [ ] TC-1h: `writeVerdict` appends the marker when absent, without disturbing other markers
      (`**Lane**`, `**Lane Status**`, `**Progress**`)
- [ ] TC-1i: `clearVerdict` removes the marker entirely; no-op when the marker is already absent
...
- [ ] TC-2a (AM-1018 reproduction): a `review` dispatch whose mock-cli both emits a `blocked`
...
- [ ] TC-2b: same as TC-2a but `**Verdict**: pass` + the `review.on_success` transition — lands
      at `quality-gate:queue`, not `review:waiting`.
- [ ] TC-2c (no verdict — regression guard): a `blocked` `post_turn_summary` with NO
...
- [ ] TC-2d (misconfigured fallback): `**Verdict**: pass` written, but the test's `workflow.json`
...
- [ ] TC-2e: a `conversation.md` comment documenting the override is present after TC-2a/TC-2b
      (not silent) — absent after TC-2c/TC-2d (those are ordinary parks, not overrides).

### Regression
- [ ] All existing cases in `conductor/tests/track-10055-waiting-any-lane.test.mjs` still pass
      unmodified.
- [ ] Any other existing test referencing `isBlockedTurn`/`extractBlockedQuestion`/
      `post_turn_summary` (`conductor/tests/stream-json-tail.test.mjs`) still passes unmodified.

## Acceptance Criteria
- [ ] All unit tests pass (Phase 1 helper tests)
- [ ] The AM-1018 reproduction (TC-2a) passes: resolved FAIL verdict + blocked annotation routes
      to `implement:queue`, not `review:waiting`
- [ ] The genuine-open-question regression guard (TC-2c) passes: no verdict marker still parks
      with a `**Waiting Reason**`
- [ ] The misconfiguration fallback (TC-2d) passes: an unroutable verdict still parks, never
      guesses
- [ ] The override is visible in `conversation.md` when it fires (TC-2e)
- [ ] No regressions in `track-10055-waiting-any-lane.test.mjs` or other existing blocked-turn
      tests
