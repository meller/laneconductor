...
- [ ] Task 1: `parseWaitingOnTracks(content)` — reads `**Waiting On Tracks**`,
      returns normalised bare track numbers (strip an `INITIALS-` prefix, strip
      leading zeros), `[]` when absent or empty.
- [ ] Task 2: `writeWaitingOnTracks(content, nums)` / `clearWaitingOnTracks(content)`
      — sparse emission, update-in-place-or-append, mirroring
      `writeWaitingReason`/`clearWaitingReason`.
- [ ] Task 3: `parseAutoResumedMarker(content)` / `writeAutoResumedMarker(content, deps)` /
      `clearAutoResumedMarker(content)` for `**Auto Resumed**: <ISO> deps=<n,n>`.
- [ ] Task 4: `resolveBlockedDependencies({ content })` — the attribution rule.
...
- [ ] Task 5: `isDependencyShipped(num, stateByTrackNumber)` — true only for
      lane `done` AND lane action status `success`. An absent entry is false
      (REQ-7, fails closed).
- [ ] Task 6: `decideAutoResume({ content, stateByTrackNumber })` →
...
- [ ] Task 7: Module header comment stating the `done:success`-not-`done`
...
- [ ] Task 1: Scan `join(process.cwd(), 'conductor', 'tracks')` (REQ-8 — primary
      ...
      reintroduce it.
- [ ] Task 2: Build `stateByTrackNumber` in one pass — `{ lane, laneActionStatus }`
      per track — so dependency lookup costs no extra directory scan.
- [ ] Task 3: For each track at `**Lane Status**: waiting`, call
      `decideAutoResume()`. Log declines at debug/quiet level, never per-cycle
      noise for the common "not attributable" case.
- [ ] Task 4: On `resume: true` — write `**Lane Status**: queue` via the existing
      ...
      marker.
- [ ] Task 5: Fan out the state change with
...
- [ ] Task 6: Append the audit comment to `conversation.md` in the required
      `> **system**: ...` format (REQ-5), naming the cleared dependencies and
      whether attribution came from the marker or was inferred.
- [ ] Task 7: `console.log` one `[reconcile-parked]` line per resume.
- [ ] Task 8: Register the `setInterval` next to the other two reconcilers, with
      a comment explaining it shares their cadence for the same reason (it walks
      the same directory).
- [ ] Task 9: Verify by actually running it — start a worker against a scratch
...
- [ ] Task 1: `ui/server/index.mjs`'s `syncTrackToFile()` — clear the
      ...
      park ends).
- [ ] Task 2: Confirm the `POST /api/projects/:id/tracks/:num/resume` path
...
- [ ] Task 3: Same clearing in the worker's own exit-handler write path, so a
...
- [ ] Task 1: Anchor `parseDependsOn()` to `/^\*\*Depends On\*\*:[ \t]*([^\n]+)/im`.
- [ ] Task 2: Anchor `waiting-state.mjs`'s `MARKER_RE` (its `MARKER_LINE_RE` is
      already anchored — the pair currently disagrees about what a marker is).
- [ ] Task 3: Audit the other `\*\*Marker\*\*:` parsers in `laneconductor.sync.mjs`
      for the same shape and anchor them; note any deliberately left unanchored
      and why.
- [ ] Task 4: Anchor the two new parsers from Phase 1 from the start.
- [ ] Task 5: Verify against this track's own pre-fix `index.md` prose (kept as
...
- [ ] Task 1: `conductor/tests/track-10086-dependency-resume.test.mjs` —
      `node:test` unit suite over the pure module. Covers AC-3, AC-4, AC-5,
      AC-8, AC-9 and every `skipReason` branch.
- [ ] Task 2: `conductor/tests/track-10086-auto-resume-e2e.test.mjs` — spawns a
...
- [ ] Task 3: Assert AC-6 and AC-7 in the e2e test — the `**Waiting Reason**`
      line is gone and exactly one `> **system**:` comment was appended.
- [ ] Task 3b: Cover REQ-10/AC-10 — a fixture `index.md` whose prose quotes both
      marker names parses as having neither.
- [ ] Task 4: Run both suites and paste real output into `conversation.md`
      before marking this phase complete.
- [ ] Task 5: Re-run the neighbouring suites that touch this machinery
...
- [ ] Task 1: Add `**Waiting On Tracks**` and the pre-existing `**Depends On**`
      to SKILL.md's marker table.
- [ ] Task 2: Add a short note to the parking guidance telling an agent that
      parks on an unshipped track to write `**Waiting On Tracks**`, so its park
      is mechanically resumable rather than needing a human.
- [ ] Task 3: Add a `conductor/product.md` paragraph next to the existing
      reconciler prose, stating what auto-resumes, what deliberately does not,
      and the `done:success`-not-`done` rule.
- [ ] Task 4: Update `conductor/constants.mjs`'s `LaneActionStatus.WAITING`
...
- [ ] Task 5: Record the recommended follow-up track for
