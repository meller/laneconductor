...
- [ ] TC-1: `{ code: 0, output: 'I am here.' }` — expected: `status: 'ok'`, `available: true`,
      `reset_at: null`.
- [ ] TC-2 (**the bug**): `{ code: 1, output: 'Failed to authenticate: OAuth session expired and
      ...
      `exhausted`.
- [ ] TC-3: `reset_at` is `null` for `auth_required` regardless of the `nowMs` passed. Calling
...
- [ ] TC-4: `last_error` for `auth_required` mentions `claude login`, and `remedy` states that
      this is the standalone CLI's own OAuth session rather than a Claude Code app session.
- [ ] TC-5: `{ code: 1, output: "You've hit your limit · resets 3pm (Europe/Berlin)" }` —
      expected: `status: 'exhausted'` with `reset_at` at 15:00 local. Preserves today's
      behaviour.
- [ ] TC-6: `resets 10:30pm` parses to 22:30 — the minute-bearing form the shared
      `isProviderExhausted` regex alone cannot handle, proving REQ-3's parse stayed in the
      classifier.
- [ ] TC-7: A parsed reset time already in the past rolls to tomorrow.
- [ ] TC-8: Exhausted but unparseable (`'quota exhausted'`, no `resets`) — expected:
      `status: 'exhausted'` with `reset_at ≈ nowMs + 15m`, not `nowMs + 60s`.
- [ ] TC-9: `{ code: 1, output: 'Error: ENOENT spawn claude' }` — expected:
      `status: 'probe_failed'`, `reset_at: null`, and `last_error` containing that output.
      Must **not** be `exhausted`.
- [ ] TC-10 (**precedence**): output containing *both* an auth phrase and the word `exhausted`
      classifies as `auth_required` — auth is checked first (REQ-2).
- [ ] TC-11 (**no false positive**): output containing a bare `401` with no auth phrasing (e.g.
      a URL or a byte count) does **not** classify as `auth_required`.
- [ ] TC-12: `isBlockingProviderStatus` — true for `exhausted`, `auth_required`,
...
- [ ] TC-12a: Every existing case in `conductor/tests/capacity-probe-throttle.test.mjs` still
      passes after `decideCapacityProbe` switches to `isBlockingProviderStatus` — run the file
      unmodified.
- [ ] TC-12b: `decideCapacityProbe` with a fresh cached `auth_required` entry — expected:
      `{ skip: true, available: false }`. Before the fix this returned `available: true`,
      because `auth_required !== 'exhausted'`.
- [ ] TC-12c: `decideCapacityProbe` with an `auth_required` entry older than the 60s TTL —
...
- [ ] TC-13: Strip layout, provider `{ status: 'auth_required' }` — renders `LOGIN REQUIRED`
      and no green/healthy dot.
- [ ] TC-14: Grid layout, same provider — badge is not `HEALTHY`, and the body names
      `claude login` and states it will not recover on its own.
- [ ] TC-15: `probe_failed` renders `PROBE FAILED` with its `last_error`, in both layouts.
- [ ] TC-16: `{ status: 'exhausted', reset_at: <future> }` still renders `EXHAUSTED` with the
...
- [ ] TC-17 (**AC-3**): A worker probing a fake CLI that always exits 1 with the OAuth-expired
...
- [ ] TC-18 (**AC-4**): Across those same cycles, `isProviderAvailable('claude')` returns false
      every time, and no dispatch is spawned. Direct regression for the burned claim/spawn
      attempt per cycle.
- [ ] TC-19 (**AC-6**): An explicit dispatch blocked by `auth_required` is marked failed with a
...
- [ ] TC-20 (**AC-8**): After the fake CLI is switched to exit 0, the next probe past the 60s
      ...
      DB edit.
- [ ] TC-21 (**AC-2**): A fake CLI emitting a genuine rate-limit message still produces
...
- [ ] TC-22: `providerBlockReason('claude')` with an `auth_required` cache entry returns a
      string naming both the provider and the login remedy; with an `exhausted` entry it names
      the reset time instead.
- [ ] TC-23 (**AC-7**): `lc status` against a project whose `provider_status` row is
