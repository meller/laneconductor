...
- [x] Phase 6: **Restarting the worker orphans any dispatch whose child process is still running** — found live 2026-08-14 dispatching `implement` for track 1114: the CLI child process (spawned by `checkDispatchInbox`) kept running and genuinely finished (real commit, real test coverage) after the sync worker process itself was restarted mid-run (PID 2102144 → 2673237, cause not yet identified — not triggered by this session's own actions at that moment). The new worker process has no reference to the old process's spawned child, so its `on('exit')` handler — which copies worktree artifacts back to the primary checkout, applies the Lane/Lane-Status merge (Phase 14 of [1114](../1114-worktrees-panel-deep-link-autopilot-cleanup/index.md)), and marks the dispatch row `done` — never runs. The dispatch row stays `claimed` forever and the board never reflects the real, completed work; only caught by manually reconciling the primary checkout's `index.md`/`test.md` and updating the dispatch row by hand. Same family as Phase 14's board-freeze bug (worktree's real state not reaching the primary checkout/DB) but a different mechanism (orphaned process listener vs. deliberately-excluded merge fields) — filed here rather than under 1114 since it's a core dispatch-lifecycle reliability gap, not specific to the Worktrees panel.
  **Fixed**: `reconcileOrphanedDispatches()` runs once per process, right after the worker's first successful registration — fetches this worker's still-`claimed` dispatches (new `GET /worker/:id/dispatch/claimed`) and, for each with a live worktree, reads the worktree's own `index.md` **Lane Status** marker (written by the agent itself, independent of whether the exit-handler machinery ever ran). First implementation attempt tried detecting the exit handler's own completion commit instead — wrong, since that commit is written BY the exit handler, the exact code that never runs in this bug, so the signal could never exist in the case it's meant to catch; caught this via direct evidence (`git log --all --grep` showed no such commit ever existed for the orphaned 1114 case) before shipping it. `copyWorktreeArtifactsToPrimary()` extracts the exit handler's inline artifact-copy so the reconciliation pass reuses the identical, already-safety-guarded code path (`conductor/services/worktree-artifact-merge.mjs`) rather than a parallel copy that could drift. 9/9 unit tests. **Live-verified**: a real worker restart correctly reconciled two dispatches genuinely orphaned earlier the same session (track 1111's `plan` and `implement` actions, stuck `claimed` since 15:00) — copied their finished work to the primary checkout and marked them `done`; track 1111 landed at `review:queue`, 95%, matching its worktree's real state exactly.
  **Notable while shipping this**: the repo's own `laneconductor.sync.mjs` and `ui/server/index.mjs` are under heavy concurrent multi-session write pressure right now — working-tree edits to these files were silently overwritten by other sessions' commits three separate times while building this fix (twice losing this fix's own uncommitted work entirely, once losing the new server endpoint). No data was lost since nothing had been committed yet each time, but it meant re-applying the same edit repeatedly and committing immediately after finishing rather than pausing to verify first. Worth its own future investigation if it keeps happening — possibly relevant to Phase 2 (single-instance guard) if multiple *workers* are somehow sharing this checkout unsafely, though this looked more like multiple *human/agent sessions* editing the same files concurrently than a worker-identity problem.
