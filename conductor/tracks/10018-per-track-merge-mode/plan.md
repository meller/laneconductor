...
## Phase 11: Subprocess-level E2E for the real worker-side PR flow (closes the last documented gap)
...
**Why this wasn't just a copy-paste of the existing pattern**: `local-api-e2e.test.mjs`'s `startWorker()` always sets `LC_SKIP_GIT_LOCK: '1'`, which skips the entire worktree-lifecycle block — exactly the code path this needs to exercise. And `reconcilePrTracks`'s 60s `setInterval` (`laneconductor.sync.mjs` line ~3785, sibling to `reconcileWorktrees`'s own at line ~3777) has no test-only override, unlike `LC_SKIP_GIT_LOCK` — a real subprocess test would otherwise need to wait a full minute per assertion, or poll flakily.

**TDD approach — do these in order, and actually run each step before moving to the next (this is the point of the exercise, not a formality)**:

- [ ] Task 1: Add `LC_RECONCILE_INTERVAL_MS` (default `60000`) as a shared override for BOTH `reconcileWorktrees`'s and `reconcilePrTracks`'s `setInterval` calls — keep them symmetric, preserving Phase 3 Task 4's "same cadence" invariant. This is pure enabling infrastructure, not the test itself — small, mechanical, low-risk.
- [ ] Task 2: Write a mock `gh` executable (`conductor/tests/mock-gh.mjs`, following `mock-cli.mjs`'s existing pattern in the same directory) that reads a small JSON script from an env var (e.g. `MOCK_GH_SCRIPT_PATH`) mapping `gh` subcommand → canned stdout/exit code, so the test can control `gh pr create`/`gh pr view`/`gh pr merge` responses deterministically without ever touching a real GitHub API. Cover at minimum: `pr create` → prints a fake PR URL; `pr view <n> --json ...` → prints the exact JSON shape `pollTrackPr` parses (`state`/`mergeStateStatus`/`statusCheckRollup`); `pr merge <n> --merge` → exit 0.
- [ ] Task 3: **Write the new test FIRST** (`conductor/tests/track-10018-pr-flow-e2e.test.mjs`) against Tasks 1-2's infrastructure, before confirming either task actually works end-to-end — reuse `track-1112-git-divergence.test.mjs`'s bare-origin-plus-clone git fixture pattern (a real throwaway `origin.git` + a `local` clone standing in for the primary checkout) so `git push -u origin track-N` has somewhere real to push to, same as `openTrackPrOnDone` actually does. Spawn the worker the same way `local-api-e2e.test.mjs`'s `startWorker()` does, but WITHOUT `LC_SKIP_GIT_LOCK`, with `LC_RECONCILE_INTERVAL_MS` set short (e.g. `500`), `PATH` prepended with the mock `gh`'s directory, and a track seeded with no `**Merge Mode**` marker (default `pr`).
- [ ] Task 4: **Run it and actually look at the failure** before writing/fixing anything — this is the "confirm red for the right reason" step TDD requires. Expect it to fail on something in the new scaffolding itself first (missing mock-gh script, wrong JSON shape, PATH not actually reaching the mock) rather than a bug in `openTrackPrOnDone`/`reconcilePrTracks` — those already have unit coverage — but don't assume that; read the actual failure.
- [ ] Task 5: Make it pass. Assert, against real state (not code review): branch pushed to the fixture's real `origin.git`, mock `gh pr create` invoked with the right args, `pr_number`/`pr_url`/`pr_status='open'` synced to the (real, in-process — reuse `mock-collector.mjs`) collector, NO local merge into `mainBranch` at this point. Then update the mock `gh`'s script mid-test to report `MERGED`, wait for the (now-fast) reconcile interval, and assert cleanup: worktree removed, local branch deleted (since the fixture can make it a real ancestor of `mainBranch` by actually merging on the fake "origin" side, mirroring what a real GitHub merge does), track transitions to `done:success`.
- [ ] Task 6: If this uncovers a real bug in the existing orchestration code (plausible — this is exactly the kind of gap integration tests catch that unit tests can't), fix it with a proper failing-test-first cycle for the fix itself, same as any other bug found during implementation, and document it in this section same as Phase 9's `project_id` bug was documented.

**Non-goals**: this does not need to cover every `pr_status` branch again (checks-failed/conflicted/closed) — `resolvePrStatus`'s unit tests already do that exhaustively; this test's job is proving the real wiring (spawn → push → gh → poll → cleanup) works end to end at least once, not re-deriving the status-mapping matrix.
