...
- [ ] TC-1: `cloud-route-parity` extracts worker collector paths from
...
- [ ] TC-2: `:param` route matching helper — expected: `/track/:num` matches
...
- [ ] TC-3: Route parity, pre-port — expected: RED, listing exactly the 11
      unmatched families from spec.md's gap table (baseline assertion; the
      count is recorded in plan.md).
- [ ] TC-4: Route parity, post-port — expected: zero unmatched paths.
- [ ] TC-5: `firebase-rewrites` with `/conductor-files` added to `WORKER_PATHS`
      and no rewrite present — expected: RED for both hosting targets.
- [ ] TC-6: Same, after the rewrite is added — expected: `/conductor-files`
      routes to the `api` function on both `app` and `landing` targets.
- [ ] TC-7: SPA paths still resolve to `index.html` after the rewrite addition
      — expected: `/`, `/board`, `/inbox` unchanged (regression guard on 10052's
      work).
- [ ] TC-8: `./scripts/migrate.sh` twice in a row — expected: first applies the
...
- [ ] TC-10: `CLAIMABLE_LANES` parity — expected: the cloud function's literal
...
- [ ] TC-19: `withTransaction` on success — expected: `BEGIN`, the callback's
      statements, `COMMIT`, and `release()` all on one client.
- [ ] TC-20: `withTransaction` when the callback throws — expected: `ROLLBACK`,
      `release()` still called, error propagates to the caller.

### Phase 3 — Coordination and read routes

- [ ] TC-21: `GET /projects/:id/workflow` with `conductor_files.workflow_json`
      set — expected: the parsed object. With it absent — expected: `{}` (no
      disk read attempted in the cloud).
- [ ] TC-22: `GET /projects/:id/workflow` for a project in another workspace —
      expected: `403` from `checkProject`, not the workflow.
- [ ] TC-23: `POST /conductor-files` — expected: `{ ok: true }` and the
      `UPDATE` scoped to the caller's own project.
- [ ] TC-24: `GET /track/:num` — expected: track fields plus a `comments`
      array; `404` for an unknown track number.
- [ ] TC-25: `POST /track/:num/lock` — expected: `{ ok: true }`, a
      `track_locks` upsert, and `tracks.lane_action_status` set to `running`;
      `404` when the track doesn't exist.
- [ ] TC-26: `POST /track/:num/unlock` — expected: the lock row deleted and
      `tracks.locked_by` cleared.
- [ ] TC-27: `POST /track/:num/prespawn-block` — expected: `{ count, kind,
...
- [ ] TC-28: `POST /tracks/claim-queue`, one queued track, two concurrent calls
      — expected: exactly one call returns the track (`FOR UPDATE SKIP LOCKED`
      running on a single client, per TC-19).
- [ ] TC-29: Claim ordering — expected: `priority DESC`, then the
      plan/review/quality-gate/other lane order, then `created_at ASC`.
- [ ] TC-30: Claim visibility — expected: `private` claims only its owner's
...
- [ ] TC-31: Targeted claim (`track_number` in body) for a track that is no
...
- [ ] TC-32: `GET /track/:num/session` with no stored row — expected:
      `{ claude_session_id: null, last_context_tokens: null, resume_count: 0 }`
      — `last_context_tokens` **null, never 0** (track 10047's cap policy
      distinguishes them).
- [ ] TC-33: `POST /track/:num/session` twice with the *same*
      `claude_session_id` — expected: `resume_count` becomes 1 then 2.
- [ ] TC-34: `POST /track/:num/session` with a *different* `claude_session_id`
      — expected: `resume_count` resets to 0.
- [ ] TC-35: `POST /track/:num/session` omitting `context_tokens` after a prior
      POST that supplied it — expected: the stored `last_context_tokens` is
      preserved, not nulled (`COALESCE`).
- [ ] TC-36: `DELETE /track/:num/session` — expected: `{ ok: true }`, and a
      following `GET` returns the empty shape from TC-32.
- [ ] TC-37: Session routes with no worker identity — expected: `400 worker
      identity required` on all three verbs.
- [ ] TC-38: `GET /worker/:id/dispatch` — expected: only `pending` entries for
      that worker, `created_at ASC`, under an `entries` key.
- [ ] TC-39: `GET /worker/:id/dispatch/claimed` — expected: only `claimed`
      entries, `claimed_at ASC`.
- [ ] TC-40: `GET /worker/:id/dispatch` where `:id` is a worker in another
      workspace — expected: `403`, empty or otherwise — never that worker's
      entries (AC-8).
- [ ] TC-41: `PATCH /worker-dispatch/:id` with `status: 'claimed'` — expected:
...
- [ ] TC-42: `GET /api/projects/:id/claimable-tracks` — expected: a track with
...
- [ ] TC-43: `GET /api/projects/:id/claimable-tracks` without `worker_id` —
