...
- [x] Task 2.1: Extend `cloud/functions/index.js`'s `auth` to resolve worker
    ...
    - [x] A token that resolves to a worker in a **different** workspace must
          `403`, not fall through as anonymous — write this test first (AC-8).
    - [x] An absent header stays valid: existing callers (the cloud UI, and
          every already-working worker route) must be unaffected.
- [x] Task 2.2: Apply the same resolution to `ui/server/index.mjs`'s
    ...
    - [x] Add a test that a local api-key-authenticated worker sending
...
- [x] Task 2.3: Send the header from the worker (REQ-3). In
      ...
      untouched.
    - [x] Do not send the header before registration returns a token — a
          request with an empty header value must not be sent at all.
- [x] Task 2.4: Add `withTransaction(fn)` to the cloud function (REQ-4):
    ...
    - [x] Unit-test rollback-on-throw and release-on-both-paths with the
...
- [x] Task 3.1: `GET /projects/:id/workflow` — `auth` + `checkProject`; read
...
- [x] Task 3.2: `POST /conductor-files` — `auth`; resolve project from
      `req.worker_project_id` or `project_id`, workspace-scoped;
      `UPDATE projects SET conductor_files = $1`.
- [x] Task 3.3: `GET /track/:num` — `auth`; track + its `track_comments`,
      `404` when absent.
- [x] Task 3.4: `POST /track/:num/lock` and `POST /track/:num/unlock` — upsert
    ...
    - [x] Two concurrent locks on one track must leave one lock row with the
...
- [x] Task 3.5: `POST /track/:num/prespawn-block` and `.../reset` — depends on
      Phase 1's migration being applied to the cloud DB.
- [x] Task 3.6: Add the `CLAIMABLE_LANES` copy to the cloud function as a named
...
- [x] Task 4.1: `POST /tracks/claim-queue` via Phase 2's `withTransaction`.
      ...
      `reason`.
    - [x] Test: single queued track, two concurrent claim calls → exactly one
          returns it (AC-5's unit-level counterpart; AC-5 itself is live).
    - [x] Test: `team` visibility grants via `worker_permissions`; `private`
          does not.
- [x] Task 4.2: `GET`/`POST`/`DELETE /track/:num/session`. Preserve exactly:
    ...
    - [x] Test each of those three semantics separately — they are what track
          10047's context-cap policy reads, and a plausible-looking rewrite that
          coerces `last_context_tokens` to `0` breaks it silently.
- [x] Task 4.3: `GET /worker/:id/dispatch` and `GET /worker/:id/dispatch/claimed`
...
- [x] Task 4.4: `PATCH /worker-dispatch/:id` — `DISPATCH_STATUSES` validation,
      ...
      caller's workspace.
- [x] Task 4.5: `GET /api/projects/:id/claimable-tracks` — port
      `resolveAssignee` and `resolvePinnedWorkers` alongside it (they are local
      module-scope helpers, not exported); `auth` + `checkProject`.
- [x] Task 4.6: Task 1.1's route-parity test should now go GREEN with zero
