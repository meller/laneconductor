...
- [ ] Task 1.1: Add `extractSessionContextTokens(logContent)` to `conductor/stream-json-tail.mjs` (REQ-2)
    - [ ] Last `assistant` event wins, mirroring `extractFinalAssistantText`'s existing shape
    - [ ] Sum `usage.cache_read_input_tokens + usage.cache_creation_input_tokens`
    - [ ] **Ignore `result` events entirely** — its `cache_read_input_tokens` is a cumulative
          cross-turn sum (2,152,229 vs a true 148,710 on `local-fs-review-10044-1788260734688.log`)
    - [ ] Return `null` (not `0`) when no assistant-level usage exists
    - [ ] Tolerate malformed JSONL lines the same way the sibling extractors do
- [ ] Task 1.2: Create `conductor/services/session-cap.mjs` with `shouldCapSession()` (REQ-1)
    - [ ] Signature `{ lastContextTokens, resumeCount, maxContextTokens, maxResumes }` → `{ cap, reason }`
    - [ ] Token check is primary; resume-count check consulted **only** when
          `lastContextTokens` is null/undefined (REQ-5)
    - [ ] A `0` threshold disables that check; unknown inputs never cap (REQ-10)
    - [ ] Header comment records *why* 400K and not the 150–200K originally proposed, with the
          p50/p90 figures — this is the number a future reader will most want justified
- [ ] Task 1.3: Write `conductor/tests/session-cap.test.mjs` (TC-1 … TC-9 in `test.md`)
- [ ] Task 1.4: Run it and confirm green, including the real-log fixture case (TC-4)
...
- [ ] Task 2.1: Atlas migration `add_session_context_bounds` (REQ-3)
    - [ ] `last_context_tokens INTEGER NULL`, `resume_count INTEGER NOT NULL DEFAULT 0`
    - [ ] Update `prisma/schema.prisma`'s `track_sessions` model to match
    - [ ] Regenerate `migrations/atlas.sum`
- [ ] Task 2.2: `GET /track/:num/session` returns the two new fields (`ui/server/index.mjs:3333`)
- [ ] Task 2.3: `POST /track/:num/session` accepts optional `context_tokens` (`ui/server/index.mjs:3346`)
    - [ ] `resume_count` increments when `claude_session_id` is unchanged, resets to `0` when it changes
    - [ ] `last_context_tokens` overwritten **only** when `context_tokens` is supplied — a POST
          without it must not erase a prior measurement (AC-9)
    - [ ] Keep the existing `req.worker_id` scoping; no client-supplied worker id
- [ ] Task 2.4: Mirror both in `conductor/tests/mock-collector.mjs`, preserving `sessionsByToken`
      per-worker scoping (REQ-7) — the worker E2E tests run against this, so a mock without
      these fields cannot exercise the cap
- [ ] Task 2.5: Extend `ui/server/tests/track-1086-sessions.test.mjs` (TC-10 … TC-13)
- [ ] Task 2.6: Run `cd ui && npm test` — confirm green, including the pre-existing 1086 and
...
- [ ] Task 3.1: Config knobs in `laneconductor.sync.mjs` (REQ-4), following the
      `LC_SPAWN_TIMEOUT_MS` precedence pattern at line 5035
    - [ ] `LC_SESSION_MAX_CONTEXT_TOKENS` → `config.worker?.session_max_context_tokens` → `400000`
    - [ ] `LC_SESSION_MAX_RESUMES` → `config.worker?.session_max_resumes` → `12`
- [ ] Task 3.2: `resolveTrackSession()` (line 5766) applies the cap (REQ-8)
    - [ ] Read `last_context_tokens` / `resume_count` off the same GET response
    - [ ] On cap: `await invalidateTrackSession(trackNumber)`, then return a fresh
          `randomUUID()` with `isFresh: true` — **reuse** the existing invalidation path, do
          not add a second one
    - [ ] Verify the downstream consequences actually follow — `sessionArgs` becomes
    ...
    - [ ] Log line per REQ-12: track, retired session id, measured value, threshold, reason
    - [ ] Do **not** post to `conversation.md` — a cap is not a terminal lane-action outcome
- [ ] Task 3.3: `persistTrackSession(trackNumber, claudeSessionId, contextTokens = null)` forwards
      `context_tokens` when present (line 5783)
- [ ] Task 3.4: Measure in `spawnCli`'s exit handler (REQ-9)
    - [ ] Reuse the `logContent` already read at line 5169 — do not re-read the log
    - [ ] Call `extractSessionContextTokens`, and when non-null report it via `persistTrackSession`
    - [ ] Best-effort throughout: a failed extraction or POST must never affect the run's
          outcome, matching how the neighbouring run-marker and claim-release cleanups behave
    - [ ] Place it so it does not fight the resume-failure invalidation at lines 5239–5242 —
          do not report a measurement for a session that was just invalidated
- [ ] Task 3.5: Confirm `local-fs` is untouched (REQ-11) — `resolveTrackSession` returns `null`
...
- [ ] Task 4.1: `conductor/tests/track-10047-bounded-resume.test.mjs` (TC-14 … TC-18)
- [ ] Task 4.2: TC-14 — over-threshold session ⇒ next dispatch cold-starts, old id gone (AC-1)
- [ ] Task 4.3: TC-15 — that cold start carries full context injection + `FRESH_SESSION: true` (AC-2).
...
- [ ] Task 4.4: TC-16 — under-threshold session still `--resume`s the same uuid (AC-3)
- [ ] Task 4.5: TC-17 — collector returning neither field ⇒ behavior identical to today (AC-6)
- [ ] Task 4.6: TC-18 — local-fs run creates and caps no session (AC-7)
- [ ] Task 4.7: **Run the full pre-existing session suite and confirm no regression** (AC-10):
...
- [ ] Task 5.1: Update `conductor/product.md`'s *"Session continuity across lane actions
      (`--resume`)"* row — continuity is bounded, and past the bound a run cold-starts with
      full file-based context re-injected (REQ-13)
- [ ] Task 5.2: Note the two config knobs and their defaults alongside the existing
      `worker.mode` / `worker.spawn_timeout_ms` documentation
- [ ] Task 5.3: Record the measurement method in `session-cap.mjs`'s header so the threshold
