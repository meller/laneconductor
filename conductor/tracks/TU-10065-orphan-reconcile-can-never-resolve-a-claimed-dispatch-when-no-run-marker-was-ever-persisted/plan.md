...
- [ ] Task 1.1: Add `KillMode=mixed` to `[Service]` with a comment naming this
...
- [ ] Task 2.1: Extend `conductor/services/run-marker.mjs` with a `finalizing`
...
- [ ] Task 2.2: Unit tests for 2.1 in
      ...
      exactly as today).
- [ ] Task 2.3: In `spawnCli`'s `proc.on('exit')`, replace the top-of-handler
...
- [ ] Task 2.4: In `reconcileOrphanedDispatchesInner`, when the marker exists and
...
- [ ] Task 3.1: Remove the `rmSync(lockPath)` from inside the no-marker fallback.
...
- [ ] Task 3.2: Resolve workspace mode for the entry using the same
      `resolveWorkspaceMode` call the dispatch path already makes
      (`:6744` / `:8588`), sourced from the primary checkout's `index.md`.
- [ ] Task 3.3: When mode is `main`, read `**Lane**`/`**Lane Status**` from the
...
- [ ] Task 3.4: Tests (see `test.md` TC-3.x), including the regression that a
...
- [ ] Task 4.1: Add `releaseOwnLocksOnShutdown()`: for each track in
...
- [ ] Task 4.2: Call it from `shutdown()` before `removeWorker()`, synchronous
...
- [ ] Task 5.3: Update `conductor/product.md`'s file-roles row for
