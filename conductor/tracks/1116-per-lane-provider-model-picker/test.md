...
- [ ] TC-0f: For a project with nothing configured, each of those 4 components
...
- [ ] TC-1: Per-lane panel renders a Provider `<select>` and a Model `<select>`
      (or datalist-backed input, matching existing conventions) — no plain
      free-text "Primary Model" input remains.
- [ ] TC-2: Provider `<select>` options match `PROVIDER_IDS` from
      `conductor/providers.mjs`.

### Feature: Defaults
- [ ] TC-3: A lane with no `primary_model` in `workflow.json` opens with
...
- [ ] TC-7: Selecting a model and saving calls `POST /api/projects/:id/workflow`
...
- [ ] TC-8: Clearing the Model selection back to "use project default" removes
...
- [ ] TC-9: `parallel_limit`, `max_retries`, `on_success`, `on_failure` fields in
      the same per-lane panel still work exactly as before this track's changes.

### Feature: Per-track model override (REQ-7)
- [ ] TC-10: `resolveLaneCliAndModel` with a track model set returns the track
      model, beating a lane `primary_model` AND the project default (unit).
- [ ] TC-11: `resolveLaneCliAndModel` with no track model behaves exactly as
      before (lane ?? project) — no regression to 1111's precedence tests.
- [ ] TC-12: A `**Model**: <id>` marker in a track's `index.md` reaches the
...
- [ ] TC-13: A track-level provider override (e.g. a stray `**Provider**:` /
...
- [ ] TC-14: Track detail panel's optional Model picker: empty = no marker
      written (inherit); selecting a model writes/updates the marker; clearing
      removes it.

### Feature: Documented limitations (REQ-8)
- [ ] TC-15: Workflow Settings help box text mentions worker-mode-only and
...
- [ ] `ConductorPanel.test.jsx` and other existing UI tests still pass (no
      regression from shared imports/hooks touched by this track).
- [ ] `npx vite build` succeeds with no new warnings introduced by this track.
