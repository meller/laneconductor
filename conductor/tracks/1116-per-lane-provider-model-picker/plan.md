...
- [ ] Task 1: Re-read `conductor/laneconductor.sync.mjs`'s `buildCliArgs` on `main`
...
- [ ] Task 2: Confirm `conductor/workflow.json` on `main` has `primary_model`
      ...
      test against.
- [ ] Task 3: Record findings in this plan (update this phase's notes) before
      proceeding to Phase 2 — if either assumption changed, update `spec.md`'s
      REQ-4 accordingly first.

**Impact**: Confirms whether Phase 2's Provider dropdown is UI-only (as spec REQ-4
assumes) or needs to write a new field.
...
- [ ] Task 1: Read `ui/server/index.mjs` around the `available_models` column
...
- [ ] Task 2: Decide fetch strategy in `WorkflowSettings.jsx`: on mount (alongside
...
- [ ] Task 3: Resolve Open Question 2 from spec.md — first-worker-reporting vs.
      merge-across-workers — pick one and document the reasoning here.

**Impact**: Determines whether Phase 3 needs a new API call or can reuse an
existing hook (check `ui/src/hooks/` for an existing workers-fetching hook before
writing a new one).
...
- [ ] Task 1: Confirm the exact shape of `project` as available in each of the
...
- [ ] Task 2: Write `ui/src/lib/defaultModel.js` exporting
...
- [ ] Task 3: Update all 5 existing call sites to use the resolver (spec
...
- [ ] Task 4: Confirm no behavioral change for the common case (a project with
...
- [ ] Task 1: Add Provider `<select>` (options from `PROVIDER_IDS`/`PROVIDERS`),
      defaulting to the project's configured `primary.cli` if known, else `claude`.
- [ ] Task 2: Add Model `<select>` (or datalist, matching existing per-lane field
...
- [ ] Task 3: Wire the lane-has-no-`primary_model` default to Phase 2b's
      `getDefaultProviderModel()` resolver (spec REQ-3c) — not a hardcoded
      `claude-sonnet-5` literal in this component.
- [ ] Task 4: Confirm pre-existing `primary_model` values (e.g. `claude-opus-5`)
      still show correctly selected on open (spec REQ-6) — do not regress this.
- [ ] Task 5: Preserve existing `updateLaneProp('primary_model', ...)` write path
      and its delete-on-empty behavior for "reset to project default".
- [ ] Task 6: Update the "Available Actions" help-box copy for `primary_model` to
...
- [ ] Task 1: Extend `resolveLaneCliAndModel` in
...
- [ ] Task 2: Marker plumbing — parse `**Model**: <id>` from `index.md` the
...
- [ ] Task 3: Extend the `stripLanePrimaryCli`-style guard to track level — a
      track-level provider override is stripped with a warning, never honored
      (same session-continuity reasoning as 1111's REQ-3).
- [ ] Task 4: Minimal UI — optional Model picker on the track detail panel,
      reusing Phase 3's picker component; empty = inherit.
- [ ] Task 5: Document REQ-8's two caveats (worker-mode-only; best-effort
...
- [ ] Task 6: Update `conductor/product.md`'s "Feature Availability —
...
- [ ] Task 0: Unit-test `getDefaultProviderModel()` directly (all 3 fallback
      tiers) — the cheapest, most isolated coverage for this track's core new
      logic.
- [ ] Task 1: Test default-to-resolver-result when lane has no `primary_model`.
- [ ] Task 2: Test pre-existing `primary_model` value shows pre-selected.
- [ ] Task 3: Test live `available_models` path (mocked workers fetch) populates
      the Model dropdown with live values, not just presets.
- [ ] Task 4: Test fallback-to-presets path when no worker reports live models.
- [ ] Task 5: Test save round-trip still writes a plain model-id string to
