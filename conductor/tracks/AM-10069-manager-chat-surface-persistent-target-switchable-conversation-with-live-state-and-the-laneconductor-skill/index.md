...
**Depends on Track 10067** ("Intelligent manager: an always-supervised, AI-capable health monitor with a watchable transcript"), specifically its REQ-14 through REQ-18: the per-project supervision pseudo-track (REQ-14), `resolveWorkerChatTarget()` returning a usable target for `type === 'manager'` instead of null (REQ-15), the `WorkerChatPanel` composer being enabled for managers and posting into that track's conversation (REQ-16), the existing "Show live session transcript" path working for a layer-2 session with no new renderer (REQ-17), and human replies in that conversation being picked up the same way as any other track (REQ-18). This track explicitly does NOT rebuild any of that — it consumes it. 10067's own Phase 5 stops at "the manager's supervisory work is watchable/chattable through the existing per-track UI"; this track starts from there and builds the persistent, target-switchable, skill-driven surface on top. Keep the boundary sharp during planning: if a requirement here turns out to need a change to the resolver's shape, the transcript event format, or the pseudo-track mechanism, that change belongs in 10067 (as a shared-contract change this track then depends on), not duplicated here.
...
**Summary**: Persistent top-level Chat view with a target switcher — manager target runs a real /laneconductor skill session via the existing conversation-reply spawn path, worker targets reuse 10037 track-scoped chat; queued (not mid-stream) intervention, deterministic setup-gap wizard gating, and live elapsed/token/status affordances from stream-json data already on the wire. 7 phases; depends on track 10067 REQ-14..REQ-18.
