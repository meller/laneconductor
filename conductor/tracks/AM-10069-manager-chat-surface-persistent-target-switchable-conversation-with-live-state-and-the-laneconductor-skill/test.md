...
### Phase 4 — Manager target contract and skill-driven turns (`conductor/tests/track-10069-manager-chat-contract.test.mjs`)

- [ ] TC-4.1 (**contract, spec.md D7**): `conductor/tracks/manager/` exists with `index.md`
      and `conversation.md` for a supervised project — expected: present; a failure here
      blocks Phase 4 and is raised on track 10067, not patched around.
- [ ] TC-4.2 (**contract**): `resolveWorkerChatTarget({ type: 'manager' }, projectId)`
      returns a non-null target — expected: non-null, resolving to the passed project.
- [ ] TC-4.3 (**contract**): a human comment written into the supervision conversation
      triggers a reply turn — expected: a spawned session against the pseudo-track within
      one worker cycle.
- [ ] TC-4.4: A manager-target message spawns through the conversation-reply path, not the
...
- [ ] TC-4.5: The opening manager turn's prompt contains the digest; the **second** message
      to the same target does not — expected: present then absent (REQ-15).
- [ ] TC-4.6: Two consecutive manager messages produce one `track_sessions` row, not two —
      expected: one row (AC-6).
- [ ] TC-4.7: `track_chat` and `worker_adhoc_chat` dispatches still complete and still
...
### Phase 5 — Queued intervention (`ui/src/components/ChatView.queued.test.jsx`)

- [ ] TC-5.1: Sending while the target has a live run marker renders a **queued** state
      naming what it is waiting on — expected: queued, with the running action named (REQ-9).
- [ ] TC-5.2: The queued state clears when the WS event stream reports the turn ended —
      expected: cleared without a user action and without a new poll (REQ-10).
- [ ] TC-5.3: Sending to an idle target never shows the queued state — expected: normal send
      path, no false queueing.
- [ ] TC-5.4: No string rendered by the composer or its hints claims the running turn is
      interrupted, stopped or cancelled — expected: assert against the rendered text
      (REQ-11).

### Phase 6 — Conditional wizard (`ui/src/components/ChatView.wizard.test.jsx`)

- [ ] TC-6.1: With a blocking gap present, opening the view renders the wizard message
      naming that gap and its remedy — expected: both strings present (REQ-17, REQ-19).
- [ ] TC-6.2: With only advisory gaps, no wizard message renders; they appear as a header
      note instead — expected: no opening message (REQ-17).
- [ ] TC-6.3: With zero gaps, opening the view issues no POST at all — expected: no
      `worker_dispatch` insert, no comment write, asserted on the mocked API (REQ-18, AC-10).
- [ ] TC-6.4: The wizard message is rendered client-side from the gap list, with no model
      turn dispatched to produce it — expected: no dispatch, asserted the same way as TC-6.3.

### Phase 7 — Real-product E2E (`ui/e2e/track-10069-chat-view.spec.js`)

- [ ] TC-7.1: Navigate to Chat, confirm the view renders with a target list and a composer
      — expected: both present in a real browser (AC-1).
- [ ] TC-7.2: Select a busy worker, confirm its live transcript renders and the composer
      targets its track — expected: transcript blocks visible (AC-8).
- [ ] TC-7.3: Against a fixture instance with a blocking gap, confirm the wizard appears;
      against a configured one, confirm it does not — expected: both branches (AC-9, AC-10).
- [ ] TC-7.4: Confirm the affordances also render on an ordinary lane-action transcript in
