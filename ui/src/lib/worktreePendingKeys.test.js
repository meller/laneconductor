...
// along with their now-deleted buttons — see worktreePendingKeys.js.
import { describe, it, expect } from 'vitest';
import { computeStillPresentKeys, computeStaleKeys, removeKey, mergeKey, completeKey, discardKey } from './worktreePendingKeys.js';
...
describe('computeStaleKeys', () => {
  ...
  it('clears a completed auto-complete pending key once the row merges and drops out of the list', () => {
    const row = { worktree_path: '/repo/.worktrees/1065', track: '1065' };
  ...
  it('clears a run-merge-action pending key once the row merges and drops out of the list', () => {
    const row = { worktree_path: '/repo/.worktrees/10018', track: '10018' };
    const pendingKeys = { [mergeKey(row)]: 1 };
    const stale = computeStaleKeys(pendingKeys, []);
    expect(stale).toEqual([mergeKey(row)]);
  });
